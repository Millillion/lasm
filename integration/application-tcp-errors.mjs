// Observe exact IO-versus-promise failures as well as ordinary half-close IO.
// A difference fails the comparison; diagnostic results retain both byte streams.
...
const inputs = ['integration/application-tcp-errors.mjs', 'integration/fixtures/TcpErrorPhases.lean'];
...
const report = { scope: 'Supplementary ordinary Lean TCP error timing, empty writes and half-close compared with both native modes; original upstream tests are untouched.',
...
try {
  ...
  report.nativeInterpreted = run('native interpreted', [lean.lean, '-Dlinter.all=false', '--run', source], project);
  assert.equal(report.nativeInterpreted.stderr, '');
  assert.ok(report.nativeInterpreted.stdout.endsWith('TCP error phases checked\n'));
  ...
  report.nativeCompiled = run('native compiled', [native], project);
  ...
  report.actual = run('deployed', [engine, ...(target === 'deno' ? ['run', '-A'] : []), join(relocated, 'main.mjs')], relocated, deployedEnvironment);
  save(); assert.deepEqual(report.actual, report.nativeInterpreted);
  report.passed = true;
