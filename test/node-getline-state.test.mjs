...
test('ordinary Lean getLine clears EOF before subsequent appended-data reads',
  { timeout: 650_000 }, async t => {
    const directory = realpathSync(mkdtempSync(join(tmpdir(), 'lasm-getline-state-')));
    ...
    assert.ok(native.stdout.endsWith('getLine stream-state comparison completed\n'));
