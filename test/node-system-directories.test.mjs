...
test('ordinary Lean system directory queries preserve native environment and buffer semantics',
  { skip: process.platform !== 'linux', timeout: 900_000 }, async t => {
    mkdirSync(join(root, '.work'), { recursive: true });
    const directory = mkdtempSync(join(root, '.work/system-directory-package-'));
    let complete = false;
    t.after(() => {
      if (complete) rmSync(directory, { recursive: true, force: true });
      else t.diagnostic(`Retained failed comparison: ${directory}`);
    });
    const fixture = join(root, 'test/fixtures/system-directories');
    const source = join(directory, 'Main.lean');
    copyFileSync(join(fixture, 'Packaged.lean'), source);
    ...
    for (const [engine, executable, args] of [
      ...
      const result = spawnSync('/usr/bin/python3', [join(fixture, 'compare.py'), output,
        engine, executable, ...args, program], { encoding: 'utf8', timeout: 240_000 });
      ...
      assert.equal(comparison.results[0].passed, true, 'Native oracle');
      assert.equal(comparison.results[1].cases.length, 15);
