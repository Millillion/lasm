...
test('ordinary Lean truncate preserves native regular-file and pipe errors',
  { skip: process.platform === 'win32', timeout: 650_000 }, async t => {
    const directory = realpathSync(mkdtempSync(join(tmpdir(), 'lasm-truncate-errors-')));
    ...
    assert.ok(native.stdout.endsWith('truncate error comparison completed\n'));
