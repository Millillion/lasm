...
function originalArguments(ffi, libc) {
  ...
  if (fd < 0) throw new Error(`Cannot read original Deno arguments: errno ${ffi.errno()}`);
  const chunks = [];
  try {
    for (;;) {
      const bytes = Buffer.alloc(4096), count = Number(read(fd, bytes, bytes.length));
      if (count < 0 && ffi.errno() === ffi.os.errno.EINTR) continue;
      if (count < 0) throw new Error(`Cannot read original Deno arguments: errno ${ffi.errno()}`);
      if (count === 0) break;
  ...
  if (start !== bytes.length || argv.length < 2) throw new Error('Invalid original Deno arguments');
  return argv;
}
