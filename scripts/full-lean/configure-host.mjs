...
for (const name of ['node.hpp', 'node-io.cpp', 'node-async.cpp']) copyFileSync(join(root, 'runtime', name), join(destination, name));
for (const name of ['host-pre.js', 'host-library.js']) copyFileSync(join(root, 'scripts/full-lean', name), join(destination, name));
const replacements = new Set();
for (const name of ['node-io.cpp', 'node-async.cpp']) {
...
const cmake = [
  'add_library(lasmhost STATIC "${LEAN_SOURCE_DIR}/lasm/node-io.cpp" "${LEAN_SOURCE_DIR}/lasm/node-async.cpp")',
