// Differential standard-stream buffering probe. Original upstream sources are not edited.
...
const evidence = { leanCommit, source, executedSource, sourceSha256: digest(bytes),
  scope: 'Supplementary standard-stream buffering and inherited-child comparison; not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && (evidence.results.length
      ? result.stdout === evidence.results[0].stdout && result.stderr === evidence.results[0].stderr
      : result.stdout.endsWith('normal shutdown flush\n') && result.stderr === 'stderr parent before|stderr child|stderr parent after\n') };
