// Supplementary native chdir comparison; upstream tests remain unchanged.
...
mkdirSync(join(output, 'search-only'), { mode: 0o111 });
...
const evidence = { leanCommit, source, sourceSha256: createHash('sha256').update(bytes).digest('hex'),
  scope: 'Supplementary POSIX chdir error and permission comparison, not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && result.stderr === '' && (evidence.results.length
      ...
      : /search denied: IO\.Error\.permissionDenied/.test(result.stdout)
        && result.stdout.endsWith('search without read: ok\ncwd error comparison completed\n')) };
...
try {
  ...
  for (const name of ['denied', 'search-only']) chmodSync(join(output, name), 0o700);
