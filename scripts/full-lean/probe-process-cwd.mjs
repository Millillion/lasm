// Supplementary POSIX path comparison; upstream tests remain unchanged.
import { existsSync, mkdirSync, symlinkSync, readFileSync, writeFileSync, createReadStream } from 'node:fs';
...
mkdirSync(join(output, 'base'));
mkdirSync(join(output, 'target/inner'), { recursive: true });
symlinkSync(join(output, 'target/inner'), join(output, 'base/link'));
...
const evidence = { leanCommit, source, sourceSha256: createHash('sha256').update(bytes).digest('hex'),
  scope: 'Supplementary POSIX child cwd comparison with absolute/relative symlink/.. paths; not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const result = spawnSync(executable, ['--run', source, output], {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && result.stderr === '' && result.stdout === 'process cwd comparison completed\n' };
