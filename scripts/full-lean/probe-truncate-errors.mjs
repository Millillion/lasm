// Differential Handle.truncate error probe. Original upstream sources are not edited.
...
if (!['linux', 'darwin'].includes(process.platform)) throw new Error('This comparison requires a POSIX host');
...
const source = join(root, 'test/fixtures/truncate-errors/Main.lean');
...
const evidence = { leanCommit, source, executedSource, sourceSha256: digest(bytes),
  scope: 'Supplementary POSIX Handle.truncate error comparison; not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && result.stderr === '' && (evidence.results.length
      ...
      : result.stdout.endsWith('truncate error comparison completed\n')) };
