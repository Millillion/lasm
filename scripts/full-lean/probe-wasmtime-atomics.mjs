...
const inputs = ['scripts/full-lean/probes/wasmtime-atomics.c', 'scripts/full-lean/probes/wasmtime-atomics.mjs',
  ...
  'scripts/full-lean/probe-wasmtime-atomics.mjs'];
const hashes = Object.fromEntries(await Promise.all(inputs.map(async path => [path, await hashFile(join(root, path))])));
const report = { scope: 'Sparse memory64 native atomic primitives, Wasm wait/notify and two-worker increments; no shipping backend or complete JS Atomics facade',
...
try {
  ...
  const worker = join(root, inputs[1]);
  for (const [engine, args] of [
    ...
    try { report.results.push(JSON.parse(run(engine, [...args, worker, helper]))); }
