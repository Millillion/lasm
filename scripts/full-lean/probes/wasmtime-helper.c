...
int lasm_probe_run(probe_t *probe, int64_t value, probe_callback_t callback,
    ...
    if (!status) {
        ...
        if (!wasmtime_instance_export_get(context, &instance, "run", 3, &item) || item.kind != WASMTIME_EXTERN_FUNC) {
            snprintf(error, size, "missing probe function"); status = 1;
        } else {
            wasmtime_val_t input = { .kind = WASMTIME_I64, .of.i64 = value }, output;
            trap = NULL;
            wasmtime_error_t *call_error = wasmtime_func_call(context, &item.of.func, &input, 1, &output, 1, &trap);
