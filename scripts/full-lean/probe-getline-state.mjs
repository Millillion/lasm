// Differential Handle.getLine stream-state probe. Original upstream sources are not edited.
...
const source = join(root, 'test/fixtures/getline-state/Main.lean');
...
const evidence = { leanCommit, source, executedSource, sourceSha256: digest(bytes),
  scope: 'Supplementary Handle.getLine EOF and append comparison; not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && result.stderr === '' && (evidence.results.length
      ...
      : result.stdout.endsWith('getLine stream-state comparison completed\n')) };
