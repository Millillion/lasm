// Differential IO.FS.realPath traversal probe. Original upstream sources are not edited.
...
import { setupRealPath } from '../../test/fixtures/realpath/setup.mjs';
...
const evidence = { leanCommit, source, executedSource, sourceSha256: digest(bytes),
  scope: 'Supplementary realPath symlink traversal, invalid bytes, and structured error comparison; not an upstream-suite result.',
...
function run(name, executable, config) {
  ...
  const result = spawnSync(executable, ['--run', executedSource, directory], {
  ...
  const record = { name, executable, config, seconds: (performance.now() - started) / 1000,
    ...
    passed: result.status === 0 && result.stderr === '' && (evidence.results.length
      ...
      : result.stdout.endsWith('realPath comparison completed\n')) };
