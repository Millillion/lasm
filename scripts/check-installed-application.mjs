...
if (!outputArg || !archiveArg || process.platform !== 'linux' || process.arch !== 'x64')
  throw new Error('Supply NEW_OUTPUT CANDIDATE_TARBALL on the Linux x64 maintainer host');
...
const oracle = join(root, '.work/managed-application-node-r2/controls.json');
...
execFileSync('/usr/bin/python3', [restrict, installationRules, '--', process.execPath, controlCopy, workspace, archive, fixtureCopy, oracleCopy,
  join(root, 'package.json')], { stdio: 'inherit', timeout: 2700_000 });
...
const result = { installation: JSON.parse(readFileSync(join(workspace, 'result.json'), 'utf8')),
  archive, archiveSha256: await hashFile(archive), deployment,
