...
const names = process.argv[3] === '--node-only' ? ['lean', 'node'] : ['lean', 'node', 'deno', 'bun'];
assert.ok(process.argv.length <= 3 || process.argv.length === 4 && process.argv[3] === '--node-only');
for (const name of names) assert.match(versions[name], /^\d+\.\d+\.\d+$/);
