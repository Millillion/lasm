...
const [outputArg, runtimeArg, previousArg, candidateVersion = '0.1.0-experimental.25', ...extra] = process.argv.slice(2);
if (!outputArg || !runtimeArg || !previousArg || extra.length || !/^0\.1\.0-experimental\.\d+$/.test(candidateVersion))
  throw new Error('Usage: package-application-release.mjs NEW_OUTPUT APPLICATION_RUNTIME PREVIOUS_PACKAGE [CANDIDATE_VERSION]');
...
// The catalog still authenticates the complete bundle below. A candidate can
// validate a newer default before the checkout's validated default is advanced.
const lean = JSON.parse(readFileSync(join(resolve(runtimeArg), 'target.json'))).lean;
if (!Object.hasOwn(toolchainCatalog.lean, lean)) throw new Error('No matching native tool catalog');
...
writeFileSync(join(staging, 'src/toolchains.json'), JSON.stringify({ ...toolchainCatalog, defaultLean: lean }, null, 2) + '\n');
...
writeFileSync(join(staging, 'examples/lean-server/lean-toolchain'), `leanprover/lean4:v${lean}\n`);
...
writeFileSync(join(staging, 'package.json'), JSON.stringify({
  name: sourcePackage.name, version: candidateVersion, private: true, license: 'UNLICENSED',
...
for (const file of ['bin/lasm.mjs', 'scripts/full-lean/host-library.js', 'src/native/node_modules/koffi/index.cjs',
  'src/native/process/manifest.json', 'src/native/bun-stack/manifest.json', 'src/native/signals/manifest.json', 'src/native-abort.cjs', `targets/${runtime.manifest.name}/target.json`])
...
const report = { scope: 'Local candidate assembly; installation and runtime checks are separate',
  ...
  candidateVersion, defaultLean: lean,
