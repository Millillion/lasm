...
**Changed:** The concurrency follow-up reached a [legacy text-parser failure](evidence/wasmtime-helper-legacy-control-2026-09-24.json). Replaced that control with tiny legacy bytecode independently checked by each stock engine before helper validation; follow-up CI is pending. The earlier three-engine helper smoke pass remains valid.
